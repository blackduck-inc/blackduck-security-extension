...
import {
  ...
  INTEGRATION_DEFAULT_BLACKDUCK_SARIF_GENERATOR_DIRECTORY,
  ...
  try {
    ...
    if (parseToBoolean(inputs.BLACKDUCKSCA_REPORTS_SARIF_CREATE)) {
      if (!IS_PR_EVENT) {
        ...
        uploadSarifResultAsArtifact(
          constants.INTEGRATION_DEFAULT_BLACKDUCK_SARIF_GENERATOR_DIRECTORY,
    ...
    if (parseToBoolean(inputs.POLARIS_REPORTS_SARIF_CREATE)) {
      if (!IS_PR_EVENT) {
        ...
        uploadSarifResultAsArtifact(
          constants.INTEGRATION_DEFAULT_POLARIS_SARIF_GENERATOR_DIRECTORY,
